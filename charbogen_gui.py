...
class CharacterGenerator:
    ...
    def reload_from_config(self, config):
        ...
        self.columns = self.load_csv_columns(self.csv_path) if self.csv_path.exists() else {}
    ...
    def load_csv_columns(self, csv_path):
        ...
        for encoding in encodings:
            try:
                ...
                last_error = e
        if rows is None:
            raise ValueError(f'CSV konnte nicht gelesen werden. Letzter Fehler: {last_error}')
        columns = {}
        for row in rows:
    ...
    def fit_and_draw(self, draw, box, text, start_size, align='left',
        ...
        while size >= min_size:
            ...
            lh = (draw.textbbox((0, 0), 'Ag', font=font)[3]
                  ...
                  + self.config.get('line_spacing', 6))
            total_h = len(all_lines) * lh

            if total_h <= max_height:
                top_pad = self.config.get('top_padding', 4)
                if valign == 'middle':
                    y = y1 + (max_height - total_h) // 2
                elif valign == 'bottom':
                    y = y2 - total_h - top_pad
                else:
                    y = y1 + top_pad
            ...
            if total_h <= max_height:
                ...
                for idx, (h_lines, d_lines) in enumerate(segments):
                    ...
                    for line in h_lines:
                        ...
                        w     = bbox[2] - bbox[0]
                        x     = (x1 + (max_width - w) // 2 if align == 'center'
                                 else x2 - w - 4 if align == 'right'
                                 else x1 + 4)
    ...
    def draw_debug_overlay(self, draw):
        ...
        for field_name, layout in self.field_layouts.items():
            ...
            draw.text((box[0] + 4, max(0, box[1] - 22)),
                      f"{field_name} {list(box)}",
                      fill=label_color, font=label_font)
        if self.attitude:
            y = self.attitude['y']
            r = self.attitude['radius']
            for i, x in enumerate(self.attitude['x_positions'], start=1):
                draw.ellipse((x-r, y-r, x+r, y+r), outline=cross_color, width=2)
                draw.text((x-8, y+r+4), str(i), fill=cross_color, font=label_font)
    ...
    def generate_mb_stats(self, rng: random.Random) -> dict:
        ...
        hp     = max(1, mods['Zähigkeit'] + rng.randint(1, 8))
    ...
    def generate_character(self, pools):
        ...
        for field_name, csv_column in self.field_mapping.items():
            if csv_column is None:
                ...
                skip_set  = {v.strip().lower()
                             for v in layout.get('skip_values', [])}
                parts = []
        ...
        for _block_name, block in self.conditional_logic.items():
                    ...
                    if trigger and trigger in source_text:
                        ...
                        if draw_col in pools and target_field:
                            ...
                            if drawn:
                                ...
                                character[target_field] = (
                                    existing + '\n' + drawn if existing else drawn)
            ...
            if 'if_field_nonempty' in block:
                ...
                field_has_content = (
                    len(val) > 0 if isinstance(val, list)
                    else bool((val or '').strip())
                )
                chosen_col = then_col if field_has_content else else_col
                character[write_to] = pools[chosen_col].draw() if chosen_col in pools else ''
    ...
    def render_sheet(self, character, rng):
        ...
        for field_name, layout in self.field_layouts.items():
            ...
            if field_name in self.scroll_fields:
                ...
                text = value.strip() if isinstance(value, str) else (str(value).strip() if value else '')
                if text:
        ...
        if self.attitude:
            ...
            cy  = self.attitude['y']
            r   = self.attitude['radius']
            lw  = self.attitude['line_width']
            draw.line((cx-r, cy-r, cx+r, cy+r), fill='black', width=lw)
            draw.line((cx-r, cy+r, cx+r, cy-r), fill='black', width=lw)
    ...
    # ------------------------------------------------------------------
    # A4-Zusammenstellung mit variablem Layout (1 / 2 / 4 pro Blatt)
    # ------------------------------------------------------------------
...
class SettingsDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        ttk.Button(
            fields_frame, text='🗺  Felder anpassen …',
        ...
        ttk.Button(btn_frame, text='Abbrechen', command=self.destroy).pack(
            side='right', padx=(6, 0))
        ttk.Button(btn_frame, text='💾  Speichern', command=self._save).pack(side='right')

    def _browse_template(self):
        path = filedialog.askopenfilename(
            parent=self, title='Template-Bild auswählen',
    ...
    def _browse_csv(self):
        path = filedialog.askopenfilename(
            parent=self, title='CSV-Datei auswählen',
...
class App(tk.Tk):
    def __init__(self, generator, initial_system=None):
        ...
        self._current_system = (
            initial_system if initial_system in SYSTEMS else system_names[0])
    ...
    def _build_ui(self):
        ...
        self._warning_bar = tk.Label(
            main, text='',
            background='#8b1a1a', foreground='white',
        ...
        system_combo = ttk.Combobox(
            top_bar, textvariable=self.system_var,
            values=list(SYSTEMS.keys()), state='readonly', width=28)
        system_combo.pack(side='left', padx=(0, 12))
        system_combo.bind('<<ComboboxSelected>>', self._on_system_change)

        ttk.Button(top_bar, text='⚙  Einstellungen',
                   command=self.open_settings).pack(side='left')

        info_text = (
            'Vorschau zeigt immer einen zufällig erzeugten Bogen. '
            'Das Seed-Feld zeigt den aktuell angezeigten Vorschau-Seed '
            'und kann auch manuell gesetzt werden.')
        ...
        ttk.Entry(bottom_bar, textvariable=self.seed_var, width=18).grid(
        ...
        ttk.Entry(bottom_bar, textvariable=self.count_var, width=8).grid(
            row=0, column=col, sticky='w', padx=(0, 12))
        col += 1

        ttk.Label(bottom_bar, text='Bögen pro Blatt:').grid(
            row=0, column=col, sticky='w', padx=(0, 4))
        col += 1
        self.sheets_per_page_var = tk.StringVar(value='4')
        spp_combo = ttk.Combobox(
            bottom_bar,
            textvariable=self.sheets_per_page_var,
            values=['1', '2', '4'],
            state='readonly',
            width=4,
        )
        spp_combo.grid(row=0, column=col, sticky='w', padx=(0, 12))
        col += 1

        ttk.Button(bottom_bar, text='Generieren',
                   command=self.generate_files).grid(
        ...
        if self.debug_enabled:
            ttk.Button(bottom_bar, text='JSON neu einlesen',
                ...
                row=0, column=col, sticky='w', padx=(12, 0))
    ...
    def _on_system_change(self, _event=None):
        ...
        config_path = APP_DIR / SYSTEMS[selected]
        try:
            ...
            messagebox.showwarning(
                ...
                f'Die Datei "{SYSTEMS[selected]}" wurde nicht gefunden.\n'
    ...
    def determine_preview_seed(self):
        seed_text = self.seed_var.get().strip()
        if not seed_text:
            return random.randint(0, 2_147_483_647), False
        try:
    ...
    def refresh_preview(self):
        ...
        try:
            ...
            self.last_auto_seed = None if user_locked else seed
    ...
    def reload_config(self):
        ...
        try:
            ...
            if self.generator.is_ready():
                ...
                self._show_missing_files_warning()
                self.status_var.set('Konfiguration gespeichert – Dateien noch nicht gefunden.')
        except Exception as e:
    ...
    def generate_files(self):
        ...
        try:
            ...
            sheets_per_page = int(self.sheets_per_page_var.get())
            if sheets_per_page not in (1, 2, 4):
                raise ValueError('Bögen pro Blatt muss 1, 2 oder 4 sein.')
